...
class Linkbird_Content_Marketing_Activator {
    ...
    public static function activate() {
        ...
        $stork = new Linkbird_Stork_Api( 'POST', '/public/plugin/status', array() );
        $response = $stork->fire();

        if ( empty( $response ) || empty( $response['body'] ) || is_wp_error( $response ) ) {
            $message = __(
                'Es konnte keine Verbindung zur linkbird Software hergestellt werden. Bitte melden Sie sich bei unserem Support.',
                'linkbird-content-marketing'
            );
            exit( $message );
        }
